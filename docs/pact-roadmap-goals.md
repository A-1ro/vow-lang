...
### 🤝 事前合意
- エラーコード採番ルール(`PACT-E[カテゴリ1桁][連番3桁]` 等)
- Diagnosticスキーマ最終形(spec v0.1 §6.2をベースにレビュー)
- ※ Diagnosticは全クレートの心臓部。ここだけは/goal前に人間レビュー必須。

### /goal
```
/goal Cargoワークスペース(pact_syntax, pact_check, pact_fmt, pact_emit,
pact_cli, pact_mcp の6クレート)が初期化され、共通Diagnostic型が
spec/diagnostic-schema.md の定義通りにpact_checkクレートに実装され、
...
### /goal
```
/goal tests/golden/syntax/ 配下の全golden test(正常系: .pact入力 →
期待ASTのJSONダンプ一致、異常系: .pact入力 → 期待Diagnostic JSON一致)が
...
### /goal
```
/goal pact_fmtが実装され、(1) tests/golden/fmt/ の入力→期待出力ペアが
...
## M3: pact_check(名前解決+型+エフェクト+契約検査)
...
(4) source mapが生成されエラー位置が.pact側の行番号に解決される
テストがパスする。cargo test全件パス。結果を表示して完了。
```

---

## M5: pact_mcp + ドッグフード実験

### 🤝 事前合意
- pact_spec の応答フォーマット(仕様セクションの構造化粒度)
- ドッグフード実験のお題設定(下記)

### /goal
```
/goal MCPサーバー(pact_spec / pact_check / pact_fmt / pact_examples)が
...
- 新規セッションのClaudeに「Pact MCPサーバーのみ」を渡し、Pactの事前知識
  ゼロの状態で課題(例: 在庫管理の関数3本を契約つきで書く)を依頼
- 成功基準: pact_check エラーゼロのコードに3往復以内に到達するか
...
- Milestone完了ごとに `pact fmt` を全コードベースに適用し、specとの乖離が
