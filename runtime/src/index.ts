// @pact/runtime — Pact が生成した TypeScript の実行時サポート。
//
// Result / Option は Pact 組み込み型のトランスパイル先(spec §5)。
...
export interface ContractViolationInfo {
  ...
  /** 契約節が書かれている .pact ファイル(リポジトリルートからの相対パス)。 */
...
export class PactContractViolation extends Error {
  readonly clause: "requires" | "ensures";
  readonly func: string;
  readonly condition: string;
  readonly file: string;
  ...
  constructor(info: ContractViolationInfo) {
    ...
    this.name = "PactContractViolation";
