...
| `.Codex-plugin/plugin.json` | `version` | 手で編集 |
| `.Codex-plugin/marketplace.json` | `plugins[].version` | 手で編集 |
...
- `.Codex-plugin/plugin.json` の `description` … 末尾の `(vX: …)` 機能一覧を新版に更新(`marketplace.json` の description は版数タグ無しなので通常そのまま)。
...
コミットメッセージ末尾は `Co-Authored-By: Codex Opus 4.8 (1M context) <noreply@anthropic.com>`、PR 本文末尾は `🤖 Generated with [Codex](https://Codex.com/Codex)`。
