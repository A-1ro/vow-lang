...
# Vow CLI installer.
#
# プラットフォームを判定し、GitHub Releases から対応する `vow` バイナリを
# ダウンロードして PATH 上のディレクトリに置く。Rust ツールチェインは不要。
#
#   curl -fsSL https://raw.githubusercontent.com/A-1ro/vow-lang/main/install.sh | sh
#
# 環境変数で上書き可能:
#   VOW_VERSION       導入するタグ(既定: 最新リリース。例: v0.1.0)
#   VOW_INSTALL_DIR   インストール先(既定: $HOME/.local/bin)
#
...
REPO="A-1ro/vow-lang"
BIN="vow"
INSTALL_DIR="${VOW_INSTALL_DIR:-$HOME/.local/bin}"
...
if [ -z "$version" ]; then
	...
	[ -n "$version" ] || err "最新バージョンを取得できません(VOW_VERSION を指定してください)"
fi

asset="vow-${target}.tar.gz"
...
mkdir -p "$INSTALL_DIR"
if install -m 0755 "$tmp/vow-${target}/${BIN}" "$INSTALL_DIR/${BIN}" 2>/dev/null; then
	:
else
	cp "$tmp/vow-${target}/${BIN}" "$INSTALL_DIR/${BIN}"
