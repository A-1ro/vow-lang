...
>
> **v0.4 / M25(#59)で、案2 にコンビネータ引数位置限定の純粋ラムダ(`p => expr` / `(a, b) => expr`)を追加した。** キャプチャ禁止・純粋限定で、引き続き第一級関数値は禁止(`let f = (ラムダ)` は `KEI-E2001`)。`examples/collections/inventory.kei` 現行版はラムダ表記を canonical な参照例として示している。本仕様の v0.1 §2.5 を併読のこと。
