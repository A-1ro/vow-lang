...
- **Draft。** 段階1の `List` は未実装(v0.1 では `List` は未定義型 `KEI-E1002`)。本文書は採用した立場と段階1の設計契約を先に固定するもの。
- **立場B を採択した(#25 の決定)。** Kei は「1 エンティティ分の純粋コア DSL」から **システム記述言語** へ射程を広げる。`List` を `Result` / `Option` と同格の **第三の組み込みジェネリクス** として段階導入する。
- 実装は `docs/kei-roadmap-goals.md` の **M9(段階1)** 以降で /goal 単位に進める。本 issue #25 は v0.3 以降のコレクション系の **親 issue(epic)**。
...
let empty = List<Int>()     // 空リスト(要素型は明示)
```

> リテラル構文(`[...]` と空リストの書き方)は段階1の /goal で文法を確定する。空リストの要素型推論は周辺文脈に依存するため、当面は明示(`List<T>()`)を基本とする。
...
> 段階1は **案2** を既定線とする(最終決定は M9 の 🤝 合意で確定)。`map`/`filter` 等に渡せるのは純粋関数のみ。エフェクト付き関数を渡す場合、その `uses` は呼び出し元へ推移伝播する(既存ルールで処理できる。§8.1)。
...
| Kei | TypeScript |
|---|---|
| `List<T>` | `readonly T[]`(不変配列) |
| `xs.length` / `xs.isEmpty` | `xs.length` / `xs.length === 0` |
| `xs.get(i)` | 範囲チェックして `Some`/`None` を返すランタイムヘルパー(`@kei/runtime`) |
| `xs.map(f)` / `xs.filter(p)` / `xs.fold(z, f)` | `xs.map(f)` / `xs.filter(p)` / `xs.reduce(f, z)` |
| `xs.all(p)` / `xs.any(p)` | `xs.every(p)` / `xs.some(p)` |

> 段階1の /goal で `@kei/runtime` に `get` 等の最小ヘルパーを追加するか、emit 側で展開するかを確定する。

## 10. ドッグフードの目標ケースの落とし所

#25 が固定を求めた 2 ケースは、立場B の段階設計で次のように落ちる(`tests/golden/check/` に現状を回帰固定済み):

| ケース | 立場B での扱い | 段階 | 現状(golden) |
|---|---|---|---|
| `totalStockValue` | **書ける**(量化子不要、`fold`) | 段階1 | `err_collection_total_stock_value`(`List` 未実装 + 関数が値でない) |
| `planAllReorders` | **書ける**(要素述語は `all`、長さは `result.length`) | 段階1〜2 | `err_collection_plan_all_reorders`(同上) |

両 golden は現在 `err_`(未実装ゆえコンパイルしない)で、**段階1 完了時に `ok_` へ移す**(人間レビュー必須の golden 変更=段階1 の受け入れ信号)。立場A なら両者は恒久的に「意図的に書けない」に落ちたはずだが、立場B では「いま書けないだけ・段階1/2で書けるようになる」と位置づける。
