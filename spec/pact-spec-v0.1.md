# Pact 言語仕様書 v0.1 (Draft)

> Code is a pact between humans and AI.
> コードは人間とAIの合意書である。

## 0. ポジショニング

Pactは「AIが書き、人間が承認し、コンパイラが履行を保証する」ことを前提に設計されたプログラミング言語。
人間の書きやすさを捨て、**検証可能性・推論の局所性・エージェントループとの親和性**に全振りする。

- ターゲットランタイム: TypeScriptへのトランスパイル(V8 / Cloudflare Workers / Node)
- ツールチェイン: `pact` CLI + Pact MCP Server を言語仕様と同格の一級市民とする
...
4. **正規形唯一** — 同じASTは常に同じテキストに整形される(`pact fmt`)。スタイル論争は仕様で殺す。diffは常に意味的差分。
...
|---|---|---|---|
| `pact_spec` | topic: string | 構造化された仕様セクション | 文法・標準ライブラリの即引きリファレンス |
| `pact_check` | source: string | Diagnostic[] (JSON) | 構文+型+エフェクト+契約の静的検査。修正候補つき |
| `pact_fmt` | source: string | 正規形source | 整形。エージェントは生成後必ず通す |
...
- 全エラーコードに `pact_spec` で引ける解説ページを対応させる。エラー→仕様→修正のループを閉じる。

## 7. CLI

```
pact check <file>     # 静的検査(JSON出力: --json)
pact fmt <file>       # 正規形整形
pact build <dir>      # TSへトランスパイル
pact test             # 契約チェック有効でテスト実行
```
...
- TSトランスパイラ
- pact fmt(正規形)
- MCPサーバー(pact_spec / pact_check / pact_fmt)
