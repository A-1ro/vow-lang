...
4. **正規形唯一** — 同じASTは常に同じテキストに整形される(`vow fmt`)。スタイル論争は仕様で殺す。diffは常に意味的差分。
...
| `Result/Option` | 専用ランタイム(`@vow/runtime`)。内部判別子 `ok` を共有し、`else fail` は両者を同じ形で分岐できる |
| `uses` | 型レベル検査はVow側で完結。TS出力にはdocコメントとして残す |
| `requires/ensures` | アサーション挿入。違反は構造化エラー `VowContractViolation`(clause/func/condition/file/line/col)を送出。`ensures` は本体をIIFEに包み戻り値(`result`)を検査、`old(expr)` は関数先頭でキャプチャ |
| tagged型 | branded type(`__vowTag`)+ 同名コンストラクタ関数 |
...
- 出力TSは人間が読める品質を保つ(デバッグ時のsource of truthはVowだが、スタックトレースの追跡可能性を確保)。
- source map(v3)対応。`sources` は .vow ファイル、`sourcesContent` 埋め込み。契約違反のエラー位置は .vow 側の行番号に解決される。
...
|---|---|---|---|
| `vow_spec` | topic: string | 構造化された仕様セクション | 文法・標準ライブラリの即引きリファレンス |
| `vow_check` | source: string | Diagnostic[] (JSON) | 構文+型+エフェクト+契約の静的検査。修正候補つき |
| `vow_fmt` | source: string | 正規形source | 整形。エージェントは生成後必ず通す |
| `vow_examples` | query: string | コード例[] | イディオム検索(「Resultの連鎖」「エフェクト宣言の書き方」等) |
| `vow_transpile` | source: string | TS source + source map | 動作確認用 |
...
```json
{
  ...
  "span": { "file": "transfer.vow", "start": {"line": 12, "col": 3}, "end": {"line": 12, "col": 28} },
  "fixes": [
...
- 全エラーコードに `vow_spec` で引ける解説ページを対応させる。エラー→仕様→修正のループを閉じる。

## 7. CLI

```
vow check <file>     # 静的検査(JSON出力: --json)
vow fmt <file>       # 正規形整形
vow build <dir>      # TSへトランスパイル
vow test             # 契約チェック有効でテスト実行
```
...
- TSトランスパイラ
- vow fmt(正規形)
- MCPサーバー(vow_spec / vow_check / vow_fmt)
