...
詳細設計(段階1のコンビネータ API、量化子なしで書ける契約の範囲、設計依存)は **`spec/kei-spec-v0.3-collections.md`(Draft)** が正本。ロードマップ上の刻みは `docs/kei-roadmap-goals.md` の **M9** 以降。issue #25 は v0.3 以降のコレクション系の親 issue。