...
//!
//! - examples/ 配下の全 .vow: `parse(fmt(parse(src))) == parse(src)`(span 除去比較)
...
fn examples_roundtrip_and_idempotent() {
    ...
    files.sort();
    assert!(!files.is_empty(), "no .vow files in {}", dir.display());

    for path in &files {
        let src = fs::read_to_string(path).expect("readable example");
        let parsed = vow_syntax::parse_module(&src);
        assert!(
        ...
        let formatted = vow_fmt::format_module(&parsed.module);
        let reparsed = vow_syntax::parse_module(&formatted);
        assert!(
        ...
        assert_eq!(
            formatted,
            vow_fmt::format_module(&reparsed.module),
...
proptest! {
    ...
    fn generated_ast_roundtrips(m in module()) {
        let src = vow_fmt::format_module(&m);
        let parsed = vow_syntax::parse_module(&src);
        prop_assert!(
            parsed.errors.is_empty(),
            "formatted output must parse cleanly: {:?}\n--- src ---\n{}",
        ...
        let again = vow_fmt::format_module(&parsed.module);
