...
//!
//! - `{name}.input.vow` を整形した結果が `{name}.expected.vow` と一致すること
//! - 期待出力そのものが正規形であること(冪等性)
//! - 整形前後で span を除く AST が一致すること(意味的変更の禁止)
//!
//! 期待ファイルの再生成: `UPDATE_GOLDEN=1 cargo test -p vow_fmt --test golden_fmt`
...
fn golden_fmt() {
    ...
    let mut cases: Vec<String> = fs::read_dir(&dir)
        ...
        .filter_map(|entry| {
            ...
            name.strip_suffix(".input.vow").map(str::to_string)
        })
        .collect();
    cases.sort();
    assert!(
    ...
    for name in &cases {
        let input =
            fs::read_to_string(dir.join(format!("{name}.input.vow"))).expect("readable input");
        let expected_path = dir.join(format!("{name}.expected.vow"));

        let actual = match vow_fmt::format_source(&input) {
            Ok(text) => text,
            Err(errors) => {
        ...
        if actual != expected {
            failures.push(format!(
                "{name}: output differs from {name}.expected.vow\n--- actual ---\n{actual}"
            ));
            continue;
        }

        // 冪等性: 期待出力はそれ自身が正規形。
        match vow_fmt::format_source(&expected) {
        ...
        let before = vow_syntax::parse_module(&input);
        let after = vow_syntax::parse_module(&actual);
