//! `vow build` のランナー。ディレクトリ単位のトランスパイルを担う。
//!
//! ARCHITECTURE.md(vow_cli 行): 言語処理は vow_emit に委譲する。ここが持つのは
//! ディレクトリ走査・all-or-nothing 制御・ファイル IO・Diagnostic の散文整形だけ。
//!
//! 振る舞い(docs/vow-roadmap-goals.md M7 事前合意):
//! - `<dir>` 配下の `**/*.vow` を再帰収集(出力先 out-dir 配下は走査しない)。
...
//! すべて stderr に出す(`vow build` を別コマンドへパイプしても汚さない)。

use std::path::{Path, PathBuf};

use vow_check::{Diagnostic, Severity};
use vow_emit::EmitOutput;

use crate::cli::UsageError;
use crate::render;

/// `vow build`。成功は終了コード 0、検査エラーは 1、ディレクトリ/IO 不正は使用法エラー。
pub fn run(dir: &Path, out_dir: Option<&Path>, source_map: bool) -> Result<u8, UsageError> {
    ...
    collect_vow_files(dir, out_canon.as_deref(), &mut files)?;
    files.sort();
    if files.is_empty() {
        return Err(UsageError(format!(
            "build: no .vow files found under '{}'",
            dir.display()
        )));
    }
    ...
    for file in &files {
        ...
        match vow_emit::emit_module(&rel, &source) {
    ...
    if !failures.is_empty() {
        ...
        eprintln!(
            "vow build: {errors} error(s) in {} file(s); no output written",
            failures.len()
        );
        return Ok(1);
    }
...
/// `<dir>` 配下を再帰し `*.vow` を集める。`skip`(out-dir の正規化パス)配下は降りない。
fn collect_vow_files(
    dir: &Path,
    skip: Option<&Path>,
    out: &mut Vec<PathBuf>,
) -> Result<(), UsageError> {
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            ...
            collect_vow_files(&path, skip, out)?;
        } else if path.extension().is_some_and(|e| e == "vow") {
            out.push(path);
        }
    }
    Ok(())
...
mod tests {
    ...
    fn rel_path_is_dir_relative_with_forward_slashes() {
        let rel = rel_path(Path::new("a/b"), Path::new("a/b/sub/m.vow"));
        assert_eq!(rel, "sub/m.vow");
    }
