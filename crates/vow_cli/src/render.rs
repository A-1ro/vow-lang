...
pub fn format_diff(file: &str, current: &str, formatted: &str) -> String {
    let mut out = format!("{file}: not formatted (run `vow fmt --write {file}`)\n");
...
mod tests {
    use super::*;
    use vow_check::{Fix, Position, Span, TextEdit};
    ...
    fn renders_all_diagnostic_elements() {
        ...
        assert!(out.contains("error[VOW-E3001]: effect 'Database.Write'"));
        assert!(out.contains("--> transfer.vow:10:10"));
