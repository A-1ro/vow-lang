//! examples/ 配下の全 .kei が構文・意味検査ともにエラーゼロであることの保証。
...
fn examples_check_clean() {
    ...
    for path in &files {
        ...
        let mut diags = kei_check::syntax_diagnostics(&rel, &parsed.errors);
        diags.extend(kei_check::check_module(&rel, &parsed.module));
        assert!(
            diags.is_empty(),
