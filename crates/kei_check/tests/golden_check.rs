...
//!
//! - `ok_*.kei` … 構文エラーゼロでパースでき、`check_module` が Diagnostic を
//!   返さないこと(expected.json は `[]`)
//! - `err_*.kei` … 1 件以上の Diagnostic を返し、その JSON が
//!   `{name}.expected.json` と一致すること
//!
//! 期待ファイルの再生成: `UPDATE_GOLDEN=1 cargo test -p kei_check --test golden_check`
...
fn check_fixture(name: &str, source: &str) -> Result<Vec<kei_check::Diagnostic>, String> {
    ...
    let file = format!("tests/golden/check/{name}.kei");
    Ok(kei_check::check_module(&file, &result.module))
}
...
fn check_diagnostics_satisfy_invariants() {
    ...
    for name in fixture_names() {
        ...
        for d in &diags {
            ...
            assert_eq!(
                d.span.file,
                format!("tests/golden/check/{name}.kei"),
...
fn all_check_categories_covered() {
    ...
    for (prefix, category) in categories {
        ...
        for name in fixtures {
            let source =
                fs::read_to_string(dir.join(format!("{name}.kei"))).expect("readable fixture");
            let diags = check_fixture(&name, &source).expect("fixture parses");
            assert!(
                diags.iter().any(|d| d.code.as_bytes()[5] == category as u8),
                "{name}: expected at least one KEI-E{category}xxx diagnostic, got: {:?}",
