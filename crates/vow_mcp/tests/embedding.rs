...
//! 変わり、`vow_spec` / `vow_examples` の応答も必ず変わる。`include_str!` は
//! 対象ファイルを rustc の依存に登録するので、`cargo test` は spec 編集後に
//! 必ず vow_mcp を再コンパイルしてからこのテストを走らせる。
//!
...
fn embedded_examples_match_disk() {
    let disk = read_tree(&workspace_root().join("examples"), "vow");
...
#[test]
fn vow_spec_serves_current_error_doc() {
    let server = Server::new();
    let disk = fs::read_to_string(workspace_root().join("spec/errors/VOW-E3001.md"))
        .expect("error doc exists");

    let request = json!({
        "jsonrpc": "2.0", "id": 1, "method": "tools/call",
        "params": { "name": "vow_spec", "arguments": { "topic": "VOW-E3001" } }
    });
    let response = server.handle(&request).expect("vow_spec responds");
    let text = served_text(&response);

    assert_eq!(
        text, disk,
        "vow_spec must serve the on-disk error doc verbatim"
    );
...
#[test]
fn vow_examples_serves_current_example_body() {
    let server = Server::new();
    let disk = fs::read_to_string(workspace_root().join("examples/contracts/withdraw.vow"))
        .expect("example exists");

    let request = json!({
        "jsonrpc": "2.0", "id": 1, "method": "tools/call",
        "params": { "name": "vow_examples", "arguments": { "query": "withdraw" } }
    });
    let response = server.handle(&request).expect("vow_examples responds");
    let text = served_text(&response);

    assert!(
        text.contains(disk.trim_end()),
        "vow_examples must serve the on-disk example body verbatim"
    );
}
