...
//!
//! 言語処理は vow_check / vow_fmt / vow_syntax に委譲し、ここでは
//! 埋め込み spec/examples の取り回しと検査結果の整形だけを行う
//! (ARCHITECTURE.md: vow_mcp は言語処理ロジックを持たない)。

use crate::embedded;

/// インライン source に与える合成ファイル名。Diagnostic の span.file に入る。
pub const SYNTHETIC_FILE: &str = "source.vow";

const MAIN_SPEC: &str = "vow-spec-v0.1.md";
...
fn spec_index() -> String {
    let mut s = String::from("# Vow spec index\n\n");
    s.push_str(
        "`vow_spec` の `topic` にセクション番号・見出しキーワード・エラーコードを渡すと\n\
         該当箇所を返す。topic を空にするとこの索引を返す。\n\n",
    );
    s.push_str("## セクション (vow-spec-v0.1.md)\n\n");
    if let Some(main) = embedded::spec_file(MAIN_SPEC) {
...
pub fn run_check(source: &str) -> ToolOutcome {
    ...
    let mut diags = vow_check::syntax_diagnostics(SYNTHETIC_FILE, &parsed.errors);
    // 構文エラーがあるときは壊れた AST に意味検査をかけない(golden_check と同方針)。
    if parsed.errors.is_empty() {
        diags.extend(vow_check::check_module(SYNTHETIC_FILE, &parsed.module));
    }
...
fn examples_index(files: &[(&str, &str)]) -> String {
    let mut s = String::from("# Vow examples\n\n");
    s.push_str(
        "`vow_examples` の `query` にキーワードを渡すと該当する例を返す\n\
