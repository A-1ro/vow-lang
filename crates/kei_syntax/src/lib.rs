//! Kei のレキサー・パーサ・AST 定義クレート。型の知識は持たない。
//!
//! エラーがあっても可能な範囲の AST と複数の [`SyntaxError`] を返す
//! (エラー回復)。構文エラーは kei_check の境界変換で Diagnostic になる
...
pub struct ParseResult {
    ...
    /// `//` 行コメントのソース順並び(M19)。フォーマッタが行ベースで
    /// leading / trailing を再構築する。AST ノードは保持しない。
...
pub fn parse_module(source: &str) -> ParseResult {
    ...
    let (mut errors, comments) = parser.into_results();
