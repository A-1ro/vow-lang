//! pact_syntax の内部エラー型 → [`Diagnostic`] の境界変換。
//!
//! ARCHITECTURE.md 不変条件 1: Diagnostic は pact_check が唯一の定義元であり、
//! 各クレートの内部エラーはこの境界で Diagnostic に写す。pact_syntax の
...
pub fn syntax_diagnostics(file: &str, errors: &[pact_syntax::SyntaxError]) -> Vec<Diagnostic> {
    errors
        .iter()
        .map(|e| {
            let fix = Fix {
...
fn convert_span(file: &str, span: pact_syntax::Span) -> Span {
    Span {
        file: file.to_string(),
        start: Position {
