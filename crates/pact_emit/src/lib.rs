//! Pact → TypeScript トランスパイラ + source map(ARCHITECTURE.md: 検査の再実装禁止)。
//!
//! 入力ソースを pact_syntax でパースし、pact_check の検査がエラーゼロの場合のみ
...
//! - tagged 型 → branded type(`__pactTag`)+ 同名コンストラクタ関数
//! - `requires` / `ensures` → `PactContractViolation`(@pact/runtime)を投げる
...
pub fn emit_module(file: &str, source: &str) -> Result<EmitOutput, Vec<Diagnostic>> {
    let parsed = pact_syntax::parse_module(source);
    let mut diags = pact_check::syntax_diagnostics(file, &parsed.errors);
    diags.extend(pact_check::check_module(file, &parsed.module));
    if !diags.is_empty() {
        return Err(diags);
    }
