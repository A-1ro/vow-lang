...
//!
//! 検査(型・名前・エフェクト)は pact_check 済みであることを前提とし、
//! ここでは構文情報のみを使って出力を組み立てる(検査の再実装禁止)。

use std::collections::BTreeSet;
use std::fmt::Write as _;

use pact_syntax::ast::{self, BinOp, UnaryOp};
use pact_syntax::Span;

use crate::sourcemap::{self, Mapping};
use crate::EmitOutput;

const RUNTIME_MODULE: &str = "@pact/runtime";
const INDENT: &str = "  ";
...
fn ts_path_for(file: &str, module: &ast::Module) -> String {
    ...
    let stem = file
        .rsplit('/')
        .next()
        .and_then(|n| n.strip_suffix(".pact"))
        .unwrap_or("module");
    format!("{stem}.ts")
}
            ...
            match item {
                ...
                ast::Item::Func(f) => {
                    ...
                    if !f.requires.is_empty() || !f.ensures.is_empty() {
                        u.names.insert("PactContractViolation");
                    }
...
impl Emitter<'_> {
    fn emit_module(&mut self) {
        self.out.line(&format!(
            "// Generated by pact_emit from {}.",
            self.src_file
        ));
        self.out
            .line("// Do not edit; regenerate with `pact build`.");
    ...
    fn emit_alias(&mut self, a: &ast::TypeAlias) {
        ...
        match &a.tag {
            Some(tag) => {
                ...
                self.out.map(a.span);
                self.out.frag(&format!(
                    "export type {name} = {underlying} & {{ readonly __pactTag: \"{tag}\" }};"
                ));
                self.out.newline();
                self.out.newline();
                self.out.line(&format!(
    ...
    fn emit_func(&mut self, f: &ast::FuncDecl) {
        ...
        for (i, expr) in old_exprs.iter().enumerate() {
            ...
            self.out.frag(&format!("const pact$old${i} = "));
        ...
        if f.ensures.is_empty() {
            ...
            self.out
                .line(&format!("const pact$result = ((): {ret} => {{"));
            self.out.indent += 1;
            self.emit_block_stmts(&f.body);
            self.out.indent -= 1;
            self.out.line("})();");
            ...
            for clause in &f.ensures {
                self.emit_contract_check(clause, "ensures", &f.name.name, Some("pact$result"));
            }
            self.in_ensures = false;
            self.out.line("return pact$result;");
        }
    ...
    fn emit_func_doc(&mut self, f: &ast::FuncDecl) {
        ...
        for c in &f.requires {
            self.out.line(&format!(" * requires {}", pact_expr_text(c)));
        }
        for c in &f.ensures {
            self.out.line(&format!(" * ensures {}", pact_expr_text(c)));
        }
        self.out.line(" */");
    }
    ...
    fn emit_contract_check(
        ...
        self.out.start_line();
        self.out.map(span);
        self.out.frag("throw new PactContractViolation({");
        self.out.newline();
        self.out.indent += 1;
        ...
        self.out.line(&format!(
            "condition: {},",
            ts_string(&pact_expr_text(clause))
        ));
        self.out
    ...
    fn emit_call(&mut self, callee: &ast::Expr, args: &[ast::Expr]) {
        if let ast::Expr::Name { name, .. } = callee {
            if name == "old" {
                ...
                self.old_counter += 1;
                self.out.frag(&format!("pact$old${i}"));
                return;
            }
        }
...
// ---------------------------------------------------------------------------
// 契約式の Pact ソース表記(condition フィールドと doc コメント用)
// ---------------------------------------------------------------------------

fn pact_bin_op(op: BinOp) -> &'static str {
    match op {
        BinOp::Eq => "==",
        BinOp::Ne => "!=",
        BinOp::Lt => "<",
...
fn pact_expr_text(e: &ast::Expr) -> String {
    ...
    match e {
        ...
        ast::Expr::Call { callee, args, .. } => {
            let args: Vec<String> = args.iter().map(pact_expr_text).collect();
        ...
        ast::Expr::RecordLit { path, fields, .. } => {
            ...
            let fs: Vec<String> = fields
                ...
                .map(|f| match &f.value {
                    ...
                    Some(v) => format!("{}: {}", f.name.name, pact_expr_text(v)),
                })
                .collect();
