//! 単一 .pact ファイルを TypeScript に変換する開発用ツール(`pact build` の最小版)。
//!
//! 使い方:
//!   cargo run -p pact_emit --example transpile -- <input.pact> [output.ts]
//!
...
fn main() -> ExitCode {
    ...
    let Some(input) = args.next() else {
        eprintln!("usage: transpile <input.pact> [output.ts]");
    ...
    match pact_emit::emit_module(&input, &source) {
        Ok(out) => match output {
            Some(path) => {
