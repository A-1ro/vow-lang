//! M4 e2e(docs/kei-roadmap-goals.md):
//! examples/ の全 .kei をトランスパイルし、
...
fn e2e_transpile_typecheck_and_run() {
    ...
    // 1. @kei/runtime を依存解決してビルド(dist/ を生成)。
    run("npm", &["install", "--no-audit", "--no-fund"], &runtime);
    run("npm", &["run", "build"], &runtime);

    // 2. e2e ハーネスの依存解決(@kei/runtime は file: で runtime/ を参照)。
    ...
    // 4. examples/ の全 .kei をトランスパイルして generated/ に書き出す。
    let mut files = Vec::new();
    collect_kei_files(&root.join("examples"), &mut files);
    files.sort();
