//! `kei test` のランナー。dev ビルド(契約 on)→ プロジェクトの `npm test` への委譲。
//!
//! ARCHITECTURE.md / docs/kei-roadmap-goals.md M7 事前合意:
//! v0.1 の Kei に test 構文は無い。`kei test` は「dev ビルド → Node のテストランナー」
...
//! `KeiContractViolation`(@kei/runtime)として送出され、捕捉しないテストは失敗 →
//! `npm test` が非ゼロ終了 → `kei test` もそれを写して非ゼロ終了する。
...
/// `kei test`。テスト全件パスで 0、テスト失敗/ビルドエラーで 1、
...
pub fn run(dir: &Path) -> Result<u8, UsageError> {
    ...
    if !dir.join("package.json").is_file() {
        return Err(UsageError(format!(
            "test: '{}' has no package.json (kei test delegates to the project's `npm test`)",
            dir.display()
        )));
    }
    ...
    eprintln!("kei test: running `npm test` in {}", dir.display());
