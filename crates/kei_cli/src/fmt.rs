//! `kei fmt` のランナー。ファイル IO と kei_fmt への委譲、出力整形だけを行う。
//!
...
//!   (kei_fmt::format_source の方針: 壊れた入力を「それらしく」書き換えない)。
//!
//! 整形済みソース(stdout)と Diagnostic / 差分(stderr)を別ストリームに分け、
//! `kei fmt foo.kei > foo.fmt` でエラー表示が混ざらないようにする。
...
/// `kei fmt`。成功時は終了コード(0 / 1)、ファイル IO 失敗は使用法エラー。
pub fn run(file: &Path, mode: FmtMode) -> Result<u8, UsageError> {
    ...
    match kei_fmt::format_source(&source) {
        ...
        Err(errors) => {
            ...
            let diags = kei_check::syntax_diagnostics(&name, &errors);
