...
//!
//! - `ok_*.vow` … 構文エラーゼロでパースでき、`check_module` が Diagnostic を
//!   返さないこと(expected.json は `[]`)
//! - `err_*.vow` … 1 件以上の Diagnostic を返し、その JSON が
//!   `{name}.expected.json` と一致すること
//!
//! 期待ファイルの再生成: `UPDATE_GOLDEN=1 cargo test -p vow_check --test golden_check`
...
fn fixture_names() -> Vec<String> {
    ...
    let mut cases: Vec<String> = fs::read_dir(&dir)
        ...
        .filter_map(|entry| {
            ...
            name.strip_suffix(".vow").map(str::to_string)
        })
        .collect();
    cases.sort();
    cases
...
fn check_fixture(name: &str, source: &str) -> Result<Vec<vow_check::Diagnostic>, String> {
    let result = vow_syntax::parse_module(source);
    ...
    let file = format!("tests/golden/check/{name}.vow");
    Ok(vow_check::check_module(&file, &result.module))
}
...
fn golden_check() {
    ...
    for name in &cases {
        let source = fs::read_to_string(dir.join(format!("{name}.vow"))).expect("readable .vow");
...
fn check_diagnostics_satisfy_invariants() {
    ...
    for name in fixture_names() {
        let source = fs::read_to_string(dir.join(format!("{name}.vow"))).expect("readable fixture");
        let diags = check_fixture(&name, &source).expect("fixture parses");
        for d in &diags {
            let code_ok = d.code.len() == 9
                && d.code.starts_with("VOW-E")
                && d.code[5..].chars().all(|c| c.is_ascii_digit())
                && matches!(d.code.as_bytes()[5], b'1'..=b'4');
            assert!(
                code_ok,
                "{name}: code must match VOW-E[1-4]xxx, got '{}'",
                d.code
            );
            assert!(
                !d.fixes.is_empty(),
            ...
            assert_eq!(
                d.span.file,
                format!("tests/golden/check/{name}.vow"),
...
fn all_check_categories_covered() {
    ...
    for (prefix, category) in categories {
        ...
        for name in fixtures {
            let source =
                fs::read_to_string(dir.join(format!("{name}.vow"))).expect("readable fixture");
            let diags = check_fixture(&name, &source).expect("fixture parses");
            assert!(
                diags.iter().any(|d| d.code.as_bytes()[5] == category as u8),
                "{name}: expected at least one VOW-E{category}xxx diagnostic, got: {:?}",
