...
//!
//! - `ok_*.vow` … エラーゼロでパースでき、AST の JSON ダンプが
//!   `{name}.expected.json` と一致すること
//! - `err_*.vow` … 1 件以上の Diagnostic を返し、その JSON が
//!   `{name}.expected.json` と一致すること
//!
//! 期待ファイルの再生成: `UPDATE_GOLDEN=1 cargo test -p vow_check --test golden_syntax`
...
fn actual_json(name: &str, source: &str) -> Result<serde_json::Value, String> {
    ...
    if name.starts_with("err_") {
        let file = format!("tests/golden/syntax/{name}.vow");
        let diags = vow_check::syntax_diagnostics(&file, &result.errors);
        if diags.is_empty() {
...
fn error_recovery_yields_multiple_diagnostics() {
    ...
    for name in ["err_multiple", "err_reserved_ident"] {
        ...
        let result = vow_syntax::parse_module(&source);
        let diags = vow_check::syntax_diagnostics(
            &format!("tests/golden/syntax/{name}.vow"),
            &result.errors,
        );
        assert!(
            diags.len() >= 2,
