...
//!
//! - examples/ 配下の全 .kei: `parse(fmt(parse(src))) == parse(src)`(span 除去比較)
...
fn examples_roundtrip_and_idempotent() {
    ...
    for path in &files {
        ...
        let formatted = kei_fmt::format_module(&parsed.module);
        let reparsed = kei_syntax::parse_module(&formatted);
        assert!(
        ...
        assert_eq!(
            formatted,
            kei_fmt::format_module(&reparsed.module),
...
proptest! {
    ...
    fn generated_ast_roundtrips(m in module()) {
        let src = kei_fmt::format_module(&m);
        let parsed = kei_syntax::parse_module(&src);
        prop_assert!(
            parsed.errors.is_empty(),
            "formatted output must parse cleanly: {:?}\n--- src ---\n{}",
        ...
        let again = kei_fmt::format_module(&parsed.module);
