...
//!
//! - `{name}.input.kei` を整形した結果が `{name}.expected.kei` と一致すること
//! - 期待出力そのものが正規形であること(冪等性)
//! - 整形前後で span を除く AST が一致すること(意味的変更の禁止)
//!
//! 期待ファイルの再生成: `UPDATE_GOLDEN=1 cargo test -p kei_fmt --test golden_fmt`
...
fn golden_fmt() {
    ...
    let mut cases: Vec<String> = fs::read_dir(&dir)
        ...
        .filter_map(|entry| {
            ...
            name.strip_suffix(".input.kei").map(str::to_string)
        })
        .collect();
    cases.sort();
    assert!(
    ...
    for name in &cases {
        let input =
            fs::read_to_string(dir.join(format!("{name}.input.kei"))).expect("readable input");
        let expected_path = dir.join(format!("{name}.expected.kei"));

        let actual = match kei_fmt::format_source(&input) {
            Ok(text) => text,
            Err(errors) => {
        ...
        if actual != expected {
            failures.push(format!(
                "{name}: output differs from {name}.expected.kei\n--- actual ---\n{actual}"
            ));
            continue;
        }

        // 冪等性: 期待出力はそれ自身が正規形。
        match kei_fmt::format_source(&expected) {
        ...
        let before = kei_syntax::parse_module(&input);
        let after = kei_syntax::parse_module(&actual);
