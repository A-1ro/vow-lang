//! M4 e2e(docs/vow-roadmap-goals.md):
//! examples/ の全 .vow をトランスパイルし、
...
fn e2e_transpile_typecheck_and_run() {
    ...
    // 1. @vow/runtime を依存解決してビルド(dist/ を生成)。
    run("npm", &["install", "--no-audit", "--no-fund"], &runtime);
    run("npm", &["run", "build"], &runtime);

    // 2. e2e ハーネスの依存解決(@vow/runtime は file: で runtime/ を参照)。
    ...
    // 4. examples/ の全 .vow をトランスパイルして generated/ に書き出す。
    let mut files = Vec::new();
    collect_vow_files(&root.join("examples"), &mut files);
    files.sort();
