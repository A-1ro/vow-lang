//! vow_emit の単体テスト: 生成 TS の形・検査エラー時の拒否・source map の解決。

use std::fs;
use std::path::{Path, PathBuf};

fn emit(src: &str) -> vow_emit::EmitOutput {
    vow_emit::emit_module("test.vow", src).unwrap_or_else(|d| {
        panic!(
...
fn enum_emits_tagged_union_and_constructors() {
    ...
    assert!(out
        .ts
        .contains("export type OrderId = string & { readonly __vowTag: \"OrderId\" };"));
    assert!(out
        .ts
...
fn requires_emits_structured_violation() {
    ...
    assert!(out.ts.contains("throw new VowContractViolation({"));
    assert!(out.ts.contains("clause: \"requires\","));
    assert!(out.ts.contains("condition: \"n > 0\","));
    assert!(out.ts.contains("file: \"test.vow\","));
    ...
    assert!(out
        .ts
        .contains("import { VowContractViolation } from \"@vow/runtime\";"));
}
...
fn ensures_wraps_body_and_captures_old() {
    ...
    assert!(out.ts.contains("const vow$old$0 = count;"));
    assert!(out.ts.contains("const vow$result = ((): number => {"));
    assert!(out.ts.contains("if (!(vow$result === vow$old$0 + step)) {"));
    assert!(out.ts.contains("clause: \"ensures\","));
    assert!(out
        .ts
        .contains("condition: \"result == old(count) + step\","));
    assert!(out.ts.contains("return vow$result;"));
}
...
fn check_errors_reject_emit() {
    let err = vow_emit::emit_module("bad.vow", "func f() -> Int {\n  return missing\n}\n")
        .expect_err("undefined name must reject emit");
    assert!(err.iter().any(|d| d.code == "VOW-E1001"));
}
...
#[test]
fn source_map_resolves_contract_violation_to_vow_line() {
    let src = concat!(
    ...
    let out = vow_emit::emit_module("contracts/demo.vow", src).expect("clean emit");

    let map: serde_json::Value = serde_json::from_str(&out.map).expect("valid JSON map");
    assert_eq!(map["version"], 3);
    assert_eq!(map["sources"][0], "contracts/demo.vow");
    assert_eq!(map["sourcesContent"][0], src);

    let decoded = decode_mappings(map["mappings"].as_str().expect("mappings string"));

    // 生成 TS で契約違反を投げる行を探し、.vow の requires 行(4行目)に解決される
    // ことを検証する。
    let throw_line = out
        .ts
        .lines()
        .position(|l| l.contains("throw new VowContractViolation"))
...
fn all_examples_emit_cleanly() {
    ...
    collect_vow_files(&dir, &mut files);
    files.sort();
    ...
    for path in &files {
        ...
        let rel = path
            ...
            .to_string_lossy()
            .replace('\\', "/");
        let out = vow_emit::emit_module(&rel, &src).unwrap_or_else(|d| {
            panic!(
