//! Vow → TypeScript トランスパイラ + source map(ARCHITECTURE.md: 検査の再実装禁止)。
//!
//! 入力ソースを vow_syntax でパースし、vow_check の検査がエラーゼロの場合のみ
...
//! - tagged 型 → branded type(`__vowTag`)+ 同名コンストラクタ関数
//! - `requires` / `ensures` → `VowContractViolation`(@vow/runtime)を投げる
...
pub fn emit_module(file: &str, source: &str) -> Result<EmitOutput, Vec<Diagnostic>> {
    ...
    let mut diags = vow_check::syntax_diagnostics(file, &parsed.errors);
    diags.extend(vow_check::check_module(file, &parsed.module));
    if !diags.is_empty() {
        return Err(diags);
    }
