...
//!
//! 検査(型・名前・エフェクト)は vow_check 済みであることを前提とし、
//! ここでは構文情報のみを使って出力を組み立てる(検査の再実装禁止)。

use std::collections::BTreeSet;
use std::fmt::Write as _;

use vow_syntax::ast::{self, BinOp, UnaryOp};
use vow_syntax::Span;

use crate::sourcemap::{self, Mapping};
use crate::EmitOutput;

const RUNTIME_MODULE: &str = "@vow/runtime";
const INDENT: &str = "  ";
            ...
            match item {
                ...
                ast::Item::Func(f) => {
                    ...
                    if !f.requires.is_empty() || !f.ensures.is_empty() {
                        u.names.insert("VowContractViolation");
                    }
...
impl Emitter<'_> {
    fn emit_module(&mut self) {
        self.out
            .line(&format!("// Generated by vow_emit from {}.", self.src_file));
        self.out
            .line("// Do not edit; regenerate with `vow build`.");
    ...
    fn emit_alias(&mut self, a: &ast::TypeAlias) {
        ...
        match &a.tag {
            Some(tag) => {
                ...
                self.out.map(a.span);
                self.out.frag(&format!(
                    "export type {name} = {underlying} & {{ readonly __vowTag: \"{tag}\" }};"
                ));
                self.out.newline();
                self.out.newline();
                self.out.line(&format!(
    ...
    fn emit_func(&mut self, f: &ast::FuncDecl) {
        ...
        for (i, expr) in old_exprs.iter().enumerate() {
            ...
            self.out.frag(&format!("const vow$old${i} = "));
        ...
        if f.ensures.is_empty() {
            ...
            self.out
                .line(&format!("const vow$result = ((): {ret} => {{"));
            self.out.indent += 1;
            self.emit_block_stmts(&f.body);
            self.out.indent -= 1;
            self.out.line("})();");
            ...
            for clause in &f.ensures {
                self.emit_contract_check(clause, "ensures", &f.name.name, Some("vow$result"));
            }
            self.in_ensures = false;
            self.out.line("return vow$result;");
        }
    ...
    fn emit_func_doc(&mut self, f: &ast::FuncDecl) {
        ...
        for c in &f.requires {
            self.out.line(&format!(" * requires {}", vow_expr_text(c)));
        }
        for c in &f.ensures {
            self.out.line(&format!(" * ensures {}", vow_expr_text(c)));
        }
        self.out.line(" */");
    }
    ...
    fn emit_contract_check(
        ...
        self.out.start_line();
        self.out.map(span);
        self.out.frag("throw new VowContractViolation({");
        self.out.newline();
        self.out.indent += 1;
        ...
        self.out.line(&format!(
            "condition: {},",
            ts_string(&vow_expr_text(clause))
        ));
        self.out
    ...
    fn emit_call(&mut self, callee: &ast::Expr, args: &[ast::Expr]) {
        if let ast::Expr::Name { name, .. } = callee {
            if name == "old" {
                ...
                self.old_counter += 1;
                self.out.frag(&format!("vow$old${i}"));
                return;
            }
        }
...
// ---------------------------------------------------------------------------
// 契約式の Vow ソース表記(condition フィールドと doc コメント用)
// ---------------------------------------------------------------------------

fn vow_bin_op(op: BinOp) -> &'static str {
    match op {
        BinOp::Eq => "==",
        BinOp::Ne => "!=",
        BinOp::Lt => "<",
...
fn vow_expr_text(e: &ast::Expr) -> String {
    ...
    match e {
        ...
        ast::Expr::Call { callee, args, .. } => {
            let args: Vec<String> = args.iter().map(vow_expr_text).collect();
        ...
        ast::Expr::RecordLit { path, fields, .. } => {
            ...
            let fs: Vec<String> = fields
                ...
                .map(|f| match &f.value {
                    ...
                    Some(v) => format!("{}: {}", f.name.name, vow_expr_text(v)),
                })
                .collect();
