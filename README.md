...
kei mcp                                          # MCP サーバーを stdio で起動(取説サーバー。下記「Kei MCP Server」参照)
```
...
```bash
# 配布版(kei CLI を入れていればこれだけ。ローカルビルド不要)
kei mcp
...
`kei mcp` と `kei-mcp` バイナリは同一の起動経路(`kei_mcp::run_stdio`)を共有します。Claude Code に登録するなら、配布版の `kei` を指すのが最短です:

```bash
claude mcp add kei-mcp -s user -- kei mcp
```
...
- [`docs/kei-roadmap-v0.4.md`](docs/kei-roadmap-v0.4.md) — v0.4 の /goal 契約書集(#54〜#62)
...
- [`HANDOFF.md`](HANDOFF.md) — 設計判断の経緯(なぜこの設計を選んだか・踏んだ地雷)。目的は intent-align、手段は DDD、結果は TDD
